...
app.post('/api/auth/register' , async (req,res) => { 
    ...
    try {
        ...
        res.status(500).json({ success: false, message: 'Failed to create user , This email or username already exist', error: error.message });
...
app.post('/api/auth/login' , async (req,res) => { 
    ...
    try {
        ...
        if (!validPassword) {
            ...
            return res.status(400).json({ success: false, message: 'Invalid password, Plase try again' });
        }
        ...
        const token = jwt.sign({id: user.id , role: user.role} , process.env.JWT_SECRET_KEY , {expiresIn: '7d'})
            ...
            }).status(200).json({success: true , message: 'Login successful with cokkie', token: token , data: user });
...
app.post('/api/tours' , verifyAdmin , async (req,res) => {
    const { title, city, address, distance, photo, description, price, maxGroupSize, featured } = req.body;

    try {
        const result = await db.query("INSERT INTO tours (title, city, address, distance, photo, description, price, maxGroupSize, featured) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING * "
        ,[title, city, address, distance, photo, description, price, maxGroupSize, featured])

        res.status(201).json({ success: true, message: 'Tour created successfully', data: result.rows[0] });
    } catch (error) {
...
app.put('/api/tours/:id' , verifyAdmin , async (req,res) => {
    ...
    try {
        const result = await db.query(
            ...
            [title, city, address, distance, photo, description, price, maxGroupSize, featured, id]
        );

        if (result.rows.length > 0) {
            res.status(200).json({ success: true, message: 'Tour updated successfully', data: result.rows[0] });
        } else {
...
app.get('/api/tours/:id', async (req, res) => {
    ...
    try {
        ...
        const reviewsResult = await db.query(`SELECT * FROM tour_reviews WHERE tour_id = $1`,[id]);
        ...
        res.status(500).json({ success: false, message: 'Failed to get single tour', error: error.message });
...
app.get('/api/tours/search/getTourBysearch', async (req, res) => {
    ...
    try {
        const result = await db.query(`SELECT tours.*, COALESCE(json_agg(tour_reviews.*), '[]') AS reviews 
            FROM tours LEFT JOIN tour_reviews ON tours.id = tour_reviews.tour_id 
        ...
        res.status(500).json({ success: false, message: 'Failed to search tours', error: error.message });
...
app.get('/api/tour/search/getTourCount', async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Failed to fetch tour count', error: error.message });
...
//booking
app.post('/api/booking', verifyUser , async (req,res) => {
